...
# First-day coach overlay.
#
# Teaches by doing: each step names one action and waits for the player to
# actually perform it, watching the same signals the game already emits. It
# never blocks input, and a pulsing box marks whatever you need to touch.
#
# Steps that have no natural trigger (nothing to "do" but read) show a button
# instead. Skip hides it for the rest of the run.
...
const STEPS: Array[Dictionary] = [
	{
		"text": "Ring the [b]bell[/b] to call the first applicant.",
		"target": "Bell",
		"signal": "customer_arrived",
	},
	{
		"text": "This is their [b]ID card[/b]. Every field on it has to obey the rules - the wrong age for a race, a sector they are barred from, a permit that runs too long.",
		"target": "Document",
		"button": "GOT IT",
	},
	{
		"text": "The [b]rulebook[/b] holds today's rules. Drag it to move it, and use [b]<[/b] and [b]>[/b] to turn the pages.",
		"target": "RuleBook",
		"signal": "rulebook_page_turned",
		# Early days have only one page, so there is nothing to turn.
		"button": "GOT IT",
	},
	{
		"text": "Decide. [b]Left-click and drag[/b] a stamp onto the card - green approves, red denies - then [b]RIGHT-CLICK[/b] to stamp it.",
		"target": "Stamp",
		"signal": "document_stamped",
	},
	{
		"text": "Now [b]drag the card onto the applicant[/b] to hand it back.",
		"target": "People",
		"signal": "document_processed",
	},
	{
		"text": "That is the job. Serve everyone who comes, then press [b]Next Day[/b]. Every call you got wrong is reviewed at the end of the day.",
		"target": "",
		"button": "START",
	},
]
...
func _ready() -> void:
	# Only on the first day, and not again once it has been seen.
	if Global.current_day != 1 or Global.tutorial_done:
		queue_free()
		return
